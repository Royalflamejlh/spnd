...
spnd can read Gemini CLI chat logs as one of its supported local data sources. Gemini uses the same unified and focused report model as Claude Code, Codex, OpenCode, Amp, pi-agent, and GitHub Copilot CLI.
...
Most users can start with unified reports such as `spnd daily`. Add the `gemini` namespace only when you want to focus the same report shape on Gemini CLI usage.
...
```bash
GEMINI_DATA_DIR="$HOME/.gemini/tmp,/backup/gemini/tmp" spnd gemini daily
```
...
| --------------------- | --------------------------------- | --------------------------------------- |
| `spnd gemini daily`   | Aggregate usage by date           | [Daily Usage](/guide/daily-reports)     |
| `spnd gemini monthly` | Aggregate usage by month          | [Monthly Usage](/guide/monthly-reports) |
| `spnd gemini session` | Group usage by Gemini CLI session | [Session Usage](/guide/session-reports) |
...
- **Cache tokens** - Gemini `input` values can include cached prompt tokens. spnd separates those cached tokens into cache read so input and cache read are not double-counted.
