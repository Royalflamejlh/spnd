...
spnd supports JSON configuration files for persistent settings. Configuration files allow you to set default options for all commands or customize behavior for specific commands without repeating options every time.
...
spnd monthly --breakdown --timezone UTC
```
...
spnd searches for configuration files in these locations (in priority order):

1. **Local project**: `.spnd/spnd.json` (higher priority)
2. **User config**: `~/.claude/spnd.json` or `~/.config/claude/spnd.json` (lower priority)

At each location, a `ccusage.json` (in a `.ccusage/` directory for the local
project variant) is also read when no `spnd.json` exists, so configuration
written for upstream ccusage keeps working unchanged.
...
Note that configuration files are not required; if none are found, spnd will use built-in defaults.
Also, if you have multiple config files, only the first one found will be used.

## Basic Configuration

Create a `spnd.json` file with your preferred defaults:
...
You can also reference a local schema file after installing spnd:

```json
{
	"$schema": "./node_modules/spnd/config-schema.json"
}
```
...
To find the exact name, run `spnd <agent> daily --json` and look at the `model` field in the per-row breakdown.
...
For a complete configuration example, see [`/spnd.example.json`](/spnd.example.json) in the repository root, which demonstrates:
...
spnd daily  # Uses mode: calculate

# But CLI argument overrides it
spnd daily --mode display  # Uses mode: display
```
...
spnd daily --debug --config ./my-config.json
```
...
```
[spnd] ℹ Debug mode enabled - showing config loading details

[spnd] ℹ Searching for config files:
  • Checking: .spnd/spnd.json (found ✓)
  • Checking: ~/.config/claude/spnd.json (found ✓)
  • Checking: ~/.claude/spnd.json (not found)

[spnd] ℹ Loaded config from: .spnd/spnd.json
  • Schema: https://ccusage.com/config-schema.json
  • Has defaults: yes (3 options)
  • Has command configs: yes (daily)

[spnd] ℹ Merging options for 'daily' command:
...
For project configs, commit `.spnd/spnd.json` to version control:

```bash
# Add to git
git add .spnd/spnd.json
git commit -m "Add spnd configuration"
```
